...
function initialListener(data) {
    ...
    if (command === 'quit') {
        ...
        console.log(
            'Unable to run "' +
                command +
                '" due to RCON not being connected yet.'
        );
    }
}
...
var poll = function () {
    ...
    ws.on('open', function open() {
        console.log(
            'Connected to RCON. Generating the map now. Please wait until the server status switches to "Running".'
        );
        waiting = false;
            ...
            if (json !== undefined) {
                if (json.Message !== undefined && json.Message.length > 0) {
                    ...
                    fs.appendFile('latest.log', '\n' + json.Message, (err) => {
                        if (err)
                            console.log('Callback error in appendFile:' + err);
                    });
                }
            } else {
